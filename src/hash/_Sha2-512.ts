 ...
 * @returns
 */
export function _sha2_512(bytes: Uint8Array, init: number[]): Uint8Array {
	const paddedBytes = pad(bytes, blockSizeBytes);

	const v = new Array<Uint64>(8);
	for (let i = 0; i < 16; i += 2) v[i >> 1] = new Uint64(init[i + 1], init[i]);
	...
	bigEndian.u64ArrIntoBytesUnsafe(v, ret);
	return ret;
}

export function sha2_512(bytes: Uint8Array): Uint8Array {
	return _sha2_512(bytes, iv512);
}
