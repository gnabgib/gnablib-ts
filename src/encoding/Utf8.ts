 ...
 */
export function bytesFromCodePoint(codePoint: number): number[] {
	if (codePoint < 0x80) {
		// eg: $ (u=x24) (8=x24)
	...
	if (codePoint < 0x800) {
		...
		//2 byte rune
		return [(codePoint >>> 6) | ind_2_byte, (codePoint & last_6_bits) | ind_cont];
	}
...
export function codePointFromBytes(bytes: number[]): number {
	if (bytes[0] >> 7 == 0) {
		...
		return (
			((bytes[0] & last_4_bits) << 12) | ((bytes[1] & last_6_bits) << 6) | (bytes[2] & last_6_bits)
		);
	}
 ...
 * @returns
 */
export function toBytes(ucs2: string): Uint8Array {
...
// export function hexHybridFromBytes(bytes:Uint8Array):string {
