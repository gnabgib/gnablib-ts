...
function xor(a: Uint32Array, b: U32ish): number {
	return a[0] ^ coerceU32(b);
}
function or(a: Uint32Array, b: U32ish): number {
	return a[0] | coerceU32(b);
}
function and(a: Uint32Array, b: U32ish): number {
	return a[0] & coerceU32(b);
}
function shl(v: Uint32Array, by: number): number {
	intExt.inRangeInclusive(by, 0, sizeBits);
	return v[0] << by;
}
function shr(v: Uint32Array, by: number): number {
	intExt.inRangeInclusive(by, 0, sizeBits);
	return (v[0] >>>= by);
}
function add(a: Uint32Array, b: U32ish): number {
	return a[0] + coerceU32(b);
}
function sub(a: Uint32Array, b: U32ish): number {
	return a[0] - coerceU32(b);
}
...
export function mul32(a: number, b: number): number {
	const b0 = b & maxU16;
	const b1 = (b >>> 16) & maxU16;
	// a*b0 = [a1*b0 | a0*b0]
	// a*b1 = [a1*b1 | a1*b0]
	return (a * b0 + ((a * b1) << 16)) >>> 0;
}

/**
 * Convert 4 bytes starting at @param pos into a U32 in little endian order (smallest byte first)
 * @param src
 * @param pos
 * @returns
 */
export function U32FromBytesLE(src: Uint8Array, pos = 0): number {
	return (
		(src[pos] |
			(src[pos + 1] << 8) |
			(src[pos + 2] << 16) |
...
/**
 * Convert 4 bytes starting at @param pos into a U32 in big endian order (smallest byte last)
 * @param src
 * @param pos
 * @returns
 */
export function U32FromBytesBE(src: Uint8Array, pos = 0): number {
	const rem = src.length - pos - sizeBytes;
	//We have to >>>0 anyway to convert to a U32.  We can reuse this when there's not enough
	// data in src, and shift the resulting magnitude down by eight (<<3) for each missing byte
	const shift = rem >= 0 ? 0 : (4 - rem) << 3;
	return (
		((src[pos] << 24) |
			(src[pos + 1] << 16) |
			(src[pos + 2] << 8) |
