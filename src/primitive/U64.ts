...
import { asBE, asE, isLE } from '../endian/platform.js';
	...
	protected shift(by:number):number[] {
		...
		return [
			...
			(byPosEq2 * this.arr[this.pos]) |
				(byPosEq1 *
					((this.arr[this.pos+1] << by32) | (by32Not0 * (this.arr[this.pos] >>> invBy32)))) |
...
export class U64MutArray {
    ...
    protected constructor(buf:Uint32Array,pos=0,len?:number) {
		...
		for(let i=0;i<len;i++) this.arr[i]=U64Mut.fromArray(this.buf,this.pos+i+i);
	...
	set(b:U64MutArray,startAt=0): void {
		this.buf.set(b.buf.subarray(startAt+startAt),this.pos);
	}

	clone():U64MutArray {
		return new U64MutArray(this.buf.slice(this.pos,this.pos+this.length+this.length),0);
	}
	 ...
	 * @returns 
	 */
	static fromBytes(buffer:ArrayBuffer,bytePos=0,len?:number):U64MutArray {
