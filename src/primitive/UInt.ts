...
export class UInt {
    ...
    protected _not() {
        for(let i=0;i<this.size;i++) this.arr[this.pos+i]=~this.arr[this.pos+i];
    }
	...
	eq(b: UInt): boolean {
        ...
        let i=0;
        if (this.size<=b.size) {
            for(;i<this.size;i++) zero |= this.arr[this.pos+i] ^ b.arr[b.pos+i];    
	 ...
	 */
	clone(): UInt {
		return new UInt(this.arr.slice(this.pos, this.pos + this.size),this.size,0);
	}
     ...
     */
    mut():UIntMut {
        return UIntMut.fromArray(this.arr.slice(this.pos, this.pos + this.size),this.size,0);
    }
...
export class UIntMut extends UInt {
	 ...
	 */
	clone(): UIntMut {
		return new UIntMut(this.arr.slice(this.pos, this.pos + this.size),this.size,0);
	}
