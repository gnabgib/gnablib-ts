...
export class BnfChar implements IBnf {
	...
	constructor(
		...
		if (typeof value === 'number') {
			...
			sLen('value', value).exactly(1).throwNot();
...
export class BnfRange implements IBnf, Iterable<BnfChar> {
	...
	constructor(
		...
		sNum('end.ord', this.end.ord).gt(this.start.ord).throwNot();
		this.name = name;
	}
...
export class BnfString implements IBnf, Iterable<BnfChar>, Iterable<IBnf> {
	...
	constructor(
		...
		if (Array.isArray(value)) {
			...
			sLen('value', value).atLeast(2).throwNot();
			let nonPrint = false;
			let ci_all_undef = true;
...
export class BnfRepeat implements IBnfRepeat {
	...
	private constructor(rule: IBnf, min: number, max: number, name?: string) {
		...
		sNum('min', min).unsigned().throwNot();
		sNum('max', max).atLeast(min).throwNot();
		this.rule = rule;
		this.min = min;
		this.max = max;
		this.name = name;
