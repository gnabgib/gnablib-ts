...
export class CreateColDef {
    ...
    static fromBin(bin:Uint8Array,pos=0):FromBinResult<CreateColDef> {
        ...
        if (!nFrom.success) return new FromBinResult<CreateColDef>(0,undefined,'CreateColDef.fromBin missing name: '+nFrom.reason);
        ptr+=nFrom.byteLen;

        const tFrom=cudColTypeFromBin(bin,ptr);
        if (!tFrom.success) return new FromBinResult<CreateColDef>(0,undefined,'CreateColDef.fromBin missing type: '+tFrom.reason);
        ptr+=tFrom.byteLen;

        return new FromBinResult(ptr-pos,new CreateColDef(nFrom.value!,tFrom.value!));
    }
}
