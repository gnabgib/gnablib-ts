...
export class TableName {
    ...
    static fromStr(name:string):TableName {
        ...
        if (bytes.length>maxLen || bytes.length<minLen)
            throw new OutOfRangeError('Length in bytes',bytes.length,minLen,maxLen);
    ...
    static fromUtf8Bytes(utf8bytes:Uint8Array):TableName {
        if (utf8bytes.length>maxLen || utf8bytes.length<minLen) 
            throw new OutOfRangeError('Length in bytes',utf8bytes.length,minLen,maxLen);
    ...
    static fromBin(bin:Uint8Array,pos=0):FromBinResult<TableName> {
        if (pos>=bin.length) 
            return new FromBinResult<TableName>(0,undefined,'TableName.fromBin unable to find length byte');

        const l=bin[pos++];
        //Confirm length is within range
        if (l>maxLen || l<minLen)
            return new FromBinResult<TableName>(0,undefined,`TableName.fromBin name size in invalid (${minLen} < ${maxLen} got ${l})`);

        const end=pos+l;
        if (end>bin.length) 
            return new FromBinResult<TableName>(0,undefined,'TableName.fromBin missing name (not enough data)');
