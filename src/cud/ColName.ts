...
export class ColName {
    ...
    static fromBin(bin:Uint8Array,pos=0):FromBinResult<ColName> {
        if (pos>=bin.length) 
            return new FromBinResult<ColName>(0,undefined,'ColName.fromBin unable to find length byte');

        const l=bin[pos++];
        //Confirm length is within range
        if (l>maxLen || l<minLen)
            return new FromBinResult<ColName>(0,undefined,`ColName.fromBin name size in invalid (${minLen} < ${maxLen} got ${l})`);

        const end=pos+l;
        if (end>bin.length) 
            return new FromBinResult<ColName>(0,undefined,'ColName.fromBin missing name (not enough data)');
