...
// prettier-ignore
/** Substitution box used in SubBytes during encryption (FIPS-197 fig 7) */
...
// prettier-ignore
/** Inverted substitution box used in SubBytes during decryption (FIPS-197 fig 14) */
const invSBox = Uint8Array.of(
...
// prettier-ignore
/** Encryption lookup table 0 (n=256) */
const te0 = Uint32Array.of(
...
// prettier-ignore
/** Encryption lookup table 1 (n=256) */
const te1 = Uint32Array.of(
...
// prettier-ignore
/** Encryption lookup table 2 (n=256) */
const te2 = Uint32Array.of(
...
// prettier-ignore
/** Encryption lookup table 4 (n=256) */
const te3 = Uint32Array.of(
...
// prettier-ignore
/** Decryption lookup table 0 (n=256) */
const td0 = Uint32Array.of(
...
// prettier-ignore
/** Decryption lookup table 1 (n=256) */
const td1 = Uint32Array.of(
...
// prettier-ignore
/** Decryption lookup table 2 (n=256) */
const td2 = Uint32Array.of(
...
// prettier-ignore
/** Decryption lookup table 3 (n=256) */
const td3 = Uint32Array.of(
...
/** Apply sBox to each byte in `u32` */
function subWord(u32: number): number {
	return (
		(sBox[u32 >>> 24] << 24) |
		(sBox[(u32 >>> 16) & 0xff] << 16) |
		(sBox[(u32 >>> 8) & 0xff] << 8) |
...
function expandKey(key: Uint8Array, enc: Uint32Array, dec: Uint32Array): void {
	...
	//Decrypt-key
	for (i = 0; i < enc.length; i += 4) {
		for (let j = 0; j < 4; j++) {
			let x = enc[enc.length - i - 4 + j];
			if (i > 0 && i + 4 < enc.length) {
				x =
					td0[sBox[x >>> 24]] ^
					td1[sBox[(x >>> 16) & 0xff]] ^
					td2[sBox[(x >>> 8) & 0xff]] ^
...
export class Aes implements ICrypt {
	...
	constructor(key: Uint8Array) {
		// 16+28=44, 24+28=52, 32+28=60
		this.#encKey = new Uint32Array(key.length + 28);
		this.#decKey = new Uint32Array(key.length + 28);
		expandKey(key, this.#encKey, this.#decKey);
	}
	...
	private _encBlock(data: Uint8Array) {
		...
		const nr1 = this.#encKey.length / 4 - 2;
		let k = 4,
			t0 = 0,
			t1 = 0,
			t2 = 0,
	...
	private _decBlock(data: Uint8Array) {
		...
		//44/4-2=9, 52/4-2=11, 60/4-2=13
		//This is 1 less than number of rounds, because the last round differs
		const nr1 = this.#decKey.length / 4 - 2;
		let k = 4,
			t0 = 0,
			t1 = 0,
			t2 = 0,
