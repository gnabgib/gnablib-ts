...
import { Key, Threefish256, Tweak } from '../sym/Threefish.js';

const defaultOutSize = 32;
...
class Skein256Core implements IHash {
	...
	constructor(digestSizeBytes: number, key: Uint8Array) {
		...
		this._tweak = Tweak.NewCfg(blockSize);
	...
	sumIn(): Uint8Array {
		...
		// this._block.fill(0);
		while (i > 0) {
			const write = Math.min(i, blockSize);
			...
			///*DBUG*/uLog64(this._b64,"b'")
			ret.set(this._block.slice(0, write), ctr * blockSize);
			ctr++;
			i -= blockSize;
...
export class Skein256 extends Skein256Core {
	constructor(sizeBytes = defaultOutSize) {
