...
import { Key, Threefish1024, Threefish256, Threefish512, Tweak } from '../sym/Threefish.js';
...
class Skein1024Core implements IHash {
	/** Block size in bytes */
	readonly blockSize = blockSize1024;
	/** Digest size in bytes */
	readonly size: number;
	/** Whether any data has been written */
	private _hasData = false;
	/** Temp processing block */
	private readonly _block = new Uint8Array(blockSize1024);
	private readonly _b64 = U64MutArray.mount(
		new Uint32Array(this._block.buffer)
	);
	/** Position of data written to block */
	...
	constructor(digestSizeBytes: number, key: Uint8Array) {
		...
		this._block[11] = digestSizeBytes >>> 21;
		this._tweak = Tweak.NewCfg(blockSize256);
		this._hVal = Key.fromSize(blockSize1024);
		///*DBUG*/uLog64(this._tweak.t64,"tweak")
		this._c = new Threefish1024(this._hVal, this._tweak);
		this.ubi();
	...
	ubi() {
        ...
        this._hVal.k64.at(6).xorEq(this._b64.at(6));
        this._hVal.k64.at(7).xorEq(this._b64.at(7));
        this._hVal.k64.at(8).xorEq(this._b64.at(8));
        this._hVal.k64.at(9).xorEq(this._b64.at(9));
        this._hVal.k64.at(10).xorEq(this._b64.at(10));
        this._hVal.k64.at(11).xorEq(this._b64.at(11));
        this._hVal.k64.at(12).xorEq(this._b64.at(12));
        this._hVal.k64.at(13).xorEq(this._b64.at(13));
        this._hVal.k64.at(14).xorEq(this._b64.at(14));
        this._hVal.k64.at(15).xorEq(this._b64.at(15));
		this._hVal.lock = false;
		///*DBUG*/uLog64(this._hVal.k64,"hVal'")
	}
	...
	sumIn(): Uint8Array {
		...
		// this._block.fill(0);
		while (i > 0) {
			const write = Math.min(i, blockSize1024);
			///*DBUG*/console.log("output")
			const c = U64.fromInt(ctr);
			this._block.fill(0);
			this._b64.at(0).set(c);
			...
			this._b64.at(0).xorEq(c);
			///*DBUG*/uLog64(this._b64,"b'")
			ret.set(this._block.slice(0, write), ctr * blockSize1024);
			ctr++;
			i -= blockSize1024;
			this._hVal.k64.set(hVal);
		}
	...
	/** Create an empty IHash using the same algorithm */
	newEmpty(): IHash {
		return new Skein1024Core(this.size, zeroU8);
	}

	/** Create a copy of the current context (uses different memory) */
	clone(): Skein1024Core {
		const r = new Skein1024Core(this.size, zeroU8);
		r._hasData = this._hasData;
		r._block.set(this._block);
		r._bPos = this._bPos;
		r._cfgBackup.set(this._cfgBackup);
...
export class Skein1024 extends Skein1024Core {
	constructor(sizeBytes = blockSize1024) {
		super(sizeBytes, zeroU8);
	}
}
