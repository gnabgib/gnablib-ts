...
function errStr(s:errorSetting):string {
	const ret=[];
	if (s.percent) {ret.push(`±${s.percent}%`);}
	if (s.amount) {ret.push(`±${s.amount}`)}
	return ret.join(', ');
}
...
export default {
	 ...
	 * @throws Error
	 */
	inClosedOpen(actual: number, lowInc: number, highExc: number): void {
		if (actual >= lowInc && actual < highExc) return;
		throw new Error(`Number '${actual}' not in range (${lowInc} <= n < ${highExc})`);
	},
	/**
	 * Confirm `found` is within `error` of `expect`
	 * @throws Error
	 */
	equalish(found:number,expect:number,error:errorSetting,name="Number"):void {
		let low=expect;
		let lowType='expect';
		let high=expect;
		let highType='expect';
