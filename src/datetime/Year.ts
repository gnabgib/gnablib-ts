...
export class Year implements ISerializer {
	...
	protected static setupStor(storage?: Uint8Array): Uint8Array {
		if (!storage) return new Uint8Array(Year.storageBytes);
		safe.len.atLeast('storage', storage, Year.storageBytes);
		return storage;
	}
	...
	public static new(v: number, storage?: Uint8Array): Year {
		...
		const stor = Year.setupStor(storage);
		Year.writeValue(stor, v + 10000);
	...
	public static newGregorian(
		...
		if (ad) {
			...
			Year.writeValue(stor, year + 10000);
		} else {
			safe.int.inRangeInc('year', year, 1, 10001);
			Year.writeValue(stor, 10001 - year);
		}
		return new Year(stor);
	}
	...
	public static fromDate(date: Date, storage?: Uint8Array): Year {
		const stor = Year.setupStor(storage);
		Year.writeValue(stor, date.getFullYear() + 10000);
		return new Year(stor);
	}
	...
	public static fromDateUtc(date: Date, storage?: Uint8Array): Year {
		const stor = Year.setupStor(storage);
		Year.writeValue(stor, date.getUTCFullYear() + 10000);
		return new Year(stor);
	}
	...
	public static parse(
		...
		if (r !== null) {
			...
			if (!isNaN(intVal)) {
				...
				return Year.new(intVal, storage);
			}
		}
	...
	public static now(storage?: Uint8Array): Year {
		return Year.fromDate(new Date(), storage);
	}
	...
	public static deserialize(source: BitReader, storage?: Uint8Array): Year {
		const stor = Year.setupStor(storage);
		Year.writeValue(stor, source.readNumber(Year.serialBits));
		return new Year(stor);
	}
}
const minY = Year.newHolocene(0);
const maxY = Year.newHolocene(32767);
