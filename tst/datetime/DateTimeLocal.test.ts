...
import { DateTimeLocal } from '../../src/datetime/dt';
import { BitWriter } from '../../src/primitive/BitWriter';
import { hex } from '../../src/codec';
import { BitReader } from '../../src/primitive/BitReader';
import util from 'util';
import { WindowStr } from '../../src/primitive/WindowStr';
import { Duration, DurationExact, IDurationExactParts, IDurationParts } from '../../src/datetime';

const tsts = suite('DateTimeLocal');
...
for (const [o, str, serStr, jsonStr] of serSet) {
	...
	const d = DateTimeLocal.new(o.y, o.m, o.d, o.h, o.i, o.s, o.us);
	tsts(`toString(${str})`, () => {
...
tsts(`new`, () => {
	const d = DateTimeLocal.new(2000, 1, 2, 11, 41, 6, 12345);
...

{
	...
	for (let h = 0; h < 24; h++) {
		...
		tsts(`fromDate(${dt})`, () => {
			const dto = DateTimeLocal.fromDate(dt);
		...
		tsts(`toDate(${dt})`, () => {
			const dto = DateTimeLocal.new(y, m, d, h, i, s, ms * 1000);
...
const fromUnixTimeMsSet: [number, string][] = [
	...
	[1705734810542, '2024-01-20T07:13:30.542000'],
	//2024-01-20 07:13:30.542789
	[1705734810542.789, '2024-01-20T07:13:30.542789'],
];
for (const [epoch, expect] of fromUnixTimeMsSet) {
	tsts(`fromUnixTimeMs(${epoch})`, () => {
		const e = DateTimeLocal.fromUnixTimeMs(epoch);
...
for (const [o, epoch, epochMs, str] of unixTimeSet) {
	...
	tsts(`fromUnixTime(${epoch})`, () => {
		const fr = DateTimeLocal.fromUnixTime(epoch);
		assert.is(fr.toString(), str);
	});
	tsts(`fromUnixTimeMs(${epochMs})`, () => {
		const fr = DateTimeLocal.fromUnixTimeMs(epochMs);
		assert.is(fr.toString(), str);
	});
}
...
tsts('[Symbol.toStringTag]', () => {
	const o = DateTimeLocal.min;
	const str = Object.prototype.toString.call(o);
	assert.is(str.indexOf('DateTimeLocal') > 0, true);
});

tsts('util.inspect', () => {
	const o = DateTimeLocal.min;
	const u = util.inspect(o);
	assert.is(u.startsWith('DateTimeLocal('), true);
});
...
tsts(`min`, () => {
	const dto = DateTimeLocal.min;
	assert.is(dto.valueOf(), '-100000101000000000000');
});
tsts(`max`, () => {
	const dto = DateTimeLocal.max;
...
const addDeSet:[string,IDurationExactParts,string][]=[
	['20230131'+'020304567890', {d:1}, '20230201'+'020304567890'],
	['20230131'+'020304567890', {h:1}, '20230131'+'030304567890'],
	['20230131'+'020304567890', {h:22}, '20230201'+'000304567890'],
	['20230131'+'020304567890', {i:1}, '20230131'+'020404567890'],
	['20230131'+'020304567890', {i:57}, '20230131'+'030004567890'],
	['20230131'+'020304567890', {s:1}, '20230131'+'020305567890'],
	['20230131'+'020304567890', {s:56}, '20230131'+'020400567890'],
	['20230131'+'020304567890', {us:1}, '20230131'+'020304567891'],
	['20230131'+'020304567890', {us:432110}, '20230131'+'020305000000'],
	['20000101'+'010203456789',{d:1000},'20020927'+'010203456789'],
	['20000101'+'010203456789',{i:59},'20000101'+'020103456789'],
];
...
const addDvSet:[string,IDurationParts,string][]=[
	['20240229'+'020304567890', {y:1}, '20250228'+'020304567890'],
	['20240229'+'020304567890', {y:4}, '20280229'+'020304567890'],
	['20240229'+'020304567890', {m:1}, '20240329'+'020304567890'],
	['20240131'+'020304567890', {m:1}, '20240229'+'020304567890'],
	['20241231'+'020304567890', {m:2}, '20250228'+'020304567890'],
	['20230131'+'020304567890', {m:1,d:1}, '20230301'+'020304567890'],
	['20240131'+'020304567890', {m:1,d:1}, '20240301'+'020304567890'],
	['20240501'+'020304567890', {m:1.5}, '20240616'+'020304567890'],// 50% of June's 30d = 15d
	['20240101'+'020304567890', {m:1.5}, '20240215'+'020304567890'],
	['20240131'+'020304567890', {m:1.5}, '20240314'+'020304567890'],// +1m=feb, 50%*29d = 14d
	['20230131'+'020304567890', {m:1}, '20230228'+'020304567890'],
	['20230131'+'020304567890', {y:1}, '20240131'+'020304567890'],
	['20230131'+'230000000000', {h:1}, '20230201'+'000000000000'],
	['20230131'+'230000000000', {m:1,h:1}, '20230301'+'000000000000'],
	['20240131'+'230000000000', {m:1,h:1}, '20240301'+'000000000000'],
	['20000101'+'010203456789',{d:1000},'20020927'+'010203456789'],
	['20000101'+'010203456789',{i:59},'20000101'+'020103456789'],
	['20240129'+'010203456789',{d:30},'20240228'+'010203456789']
];
...
const subDvSet:[string,IDurationParts,string][]=[
	...
	['20240228'+'010203456789',{d:30},'20240129'+'010203456789'],
];
...
for(const [a,b,du] of diffSet) {
	...
	tsts(`${dtb} - ${dta}`,()=>{
		assert.equal(dtb.diff(dta).toString(),du);
	})
}

const diffExactSet:[string,string,string][]=[
	['20240228'+'020304567890','20240228'+'020304567890','0s'],
	['20000101'+'010203456789','19970406'+'010203456789','1000d'],
	['20020927'+'010203456789','20000101'+'010203456789','1000d'],
...
for(const [a,b,du] of diffExactSet) {
	...
	tsts(`${dtb} - ${dta}`,()=>{
		assert.equal(dtb.diffExact(dta).toString(),du);
	})
}
...
const parseSet: [WindowStr, string, number][] = [
	...
	[WindowStr.new('20010203235959999999'), '2001-02-03T23:59:59.999999', 0],
	[WindowStr.new('20230131020304567890'), '2023-01-31T02:03:04.567890', 0],
	[WindowStr.new('227671231235959999999'), '+22767-12-31T23:59:59.999999', 0], //max

	//T separated numbers
	[WindowStr.new('-100000101T000000000000'), '-10000-01-01T00:00:00.000000', 0], //min
	[WindowStr.new('20010203T235959999999'), '2001-02-03T23:59:59.999999', 0],
	[WindowStr.new('20230131T020304567890'), '2023-01-31T02:03:04.567890', 0],
	[WindowStr.new('+227671231T235959999999'), '+22767-12-31T23:59:59.999999', 0], //max
	[WindowStr.new('227671231T235959999999'), '+22767-12-31T23:59:59.999999', 0], //max
	...
	[WindowStr.new('2001-02-03T235959999999'), '2001-02-03T23:59:59.999999', 0],
	...
	[WindowStr.new('20010203T23:59:59.999999'), '2001-02-03T23:59:59.999999', 0],
	...
	[
		WindowStr.new('2001-02-03T23:59:59.999999'),
		'2001-02-03T23:59:59.999999',
		0,
	],
	[
		WindowStr.new('2023-01-31T02:03:04.567890'),
		'2023-01-31T02:03:04.567890',
		0,
	],
	[
	...
	//RFC3339
	[WindowStr.new('1985-04-12T23:20:50.52'), '1985-04-12T23:20:50.520000', 0],
];
for (const [w, expect, expectLen] of parseSet) {
	tsts(`parse(${w.debug()})`, () => {
		const to = DateTimeLocal.parse(w);
...
for (const w of badParseStrictSet) {
	tsts(`parse-strict ${w.debug()} throws`, () => {
		assert.throws(() => DateTimeLocal.parse(w, true));
	});
}

const badParseSet: WindowStr[] = [
	WindowStr.new('1985-04-12T23:20:50.52z'),//z not allowed
	WindowStr.new('1536'),
	WindowStr.new('15:36:.'),
...
for (const w of badParseSet) {
	tsts(`parse ${w.debug()} throws`, () => {
		assert.throws(() => DateTimeLocal.parse(w));
	});
}

tsts(`parse-now`, () => {
	const w = WindowStr.new('now');
	const dt = DateTimeLocal.parse(w);
