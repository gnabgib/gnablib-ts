...
import { Gjrand32b } from '../../src/prng/gjrand32';

const tsts = suite('Gjrand32b');
...
const rng_42=Gjrand32b.seed(42);
i=0;
for (const expect of seq_42) {
    const act = rng_42.rawNext();
    tsts(`Gjrand32b(42).rawNext[${i}]`, () => {
        assert.equal(act,expect);
    });
    i++;
}
...
const rng_64=Gjrand32b.seed(987654321,123456789);
i=0;
for (const expect of seq_64) {
    const act = rng_64.rawNext();
    tsts(`Gjrand32b(987654321,123456789).rawNext[${i}]`, () => {
        assert.equal(act,expect);
    });
    i++;
}

tsts(`Gjrand32b(,false) save returns empty, restore throws`,()=>{
    const r=Gjrand32b.new();
    const sav=r.save();
    assert.equal(sav.length,0);
    assert.throws(()=>Gjrand32b.restore(sav));
})

tsts(`Gjrand32b().save/restore loop`,()=>{
    const r=Gjrand32b.new(true);
    ...
    const r2=Gjrand32b.restore(sav);    
    assert.equal(r2.rawNext(),seq_def[1],'r2 still at [1]');
    assert.equal(r.rawNext(),seq_def[3],'r consume[3]');

    assert.is(Object.prototype.toString.call(r).indexOf("gjrand32b")>0,true,'toString is set');
});
