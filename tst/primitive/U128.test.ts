...
import { U128 } from '../../src/primitive/U128';
...
for(const [aInt,bInt,expect] of eqTest) {
	tsts(`U128{${aInt}}==U128{${bInt}}`,()=>{
		const a=U128.fromInt(aInt);
		const b=U128.fromInt(bInt);
...
tsts(`constants`,()=>{
	...
	assert.equal(U128.min.toBytesBE(),new Uint8Array(16));
