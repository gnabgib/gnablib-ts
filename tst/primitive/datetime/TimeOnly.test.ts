...
//You may wonder why this isn't exposed by TimeOnly: you're not supposed
// to provide storage, but sub systems (eg TimeOnly to Hours, DateTime to TimeOnly)
// within the package do use it
const storeBytes=7;
const stor=new Uint8Array(storeBytes);
...
tsts(`fromDate-provide storage`,()=>{
    const dt=new Date(2001,1/*=2 for fucks sake JS*/,3,4,5,6,789);
    const t=TimeOnly.fromDate(dt,false,stor);
    assert.is(t.valueOf(),40506789000);
});

tsts(`fromMicrosecondsFloat`,()=>{
    const dt=new Date(2001,1/*=2 for fucks sake JS*/,3,4,5,6,789);
    const ms=dt.valueOf()+0.123;
    const t=TimeOnly.fromMicrosecondsFloat(ms);
    assert.is(t.toString(),'04:05:06.789123');
});
tsts(`fromMicrosecondsFloat-provide storage`,()=>{
    
    const dt=new Date(2001,1/*=2 for fucks sake JS*/,3,4,5,6,789);
    const ms=dt.valueOf()+0.123;
    const t=TimeOnly.fromMicrosecondsFloat(ms,true,stor);
    assert.is(t.toString(),'04:05:06.789123Z');
    assert.is(t.valueOf(),40506789123);
});
