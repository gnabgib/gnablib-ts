...
for (const [a,b,expect] of xorTest) {
	tsts(`${a} ^ ${b}`, () => {
		...
		const aUint = UIntMut.fromBytesBE(aBytes,aBytes.length>>2);
		const bBytes = hex.toBytes(b);
		const bUint = UInt.fromBytesBE(bBytes,bBytes.length>>2);
		const res = aUint.xorEq(bUint);
...
for (const [a,b,size,expect] of subTest) {
	tsts(`${a} - ${b}`, () => {
		const aUint = UIntMut.fromInt(a,size);
...
for (const [a,b,expect] of mulTest) {
	tsts(`${a} * ${b}`, () => {
		...
		const bBytes = hex.toBytes(b);
		const bUint = UInt.fromBytesBE(bBytes,bBytes.length>>2);
		const res = aUint.mulEq(bUint);
		assert.is(hex.fromBytes(res.toBytesBE()), expect);
	});
}

tsts(`fromUint32Set`,()=>{
    const a=UIntMut.fromUint32Set(0x76543210, 0xfedcba98);
...
for (const [b,expect] of setTest) {
    tsts(`set=${expect}`,()=>{
        const a=UIntMut.fromUint32Set(0xffffffff,0xffffffff,0xffffffff);
        assert.equal(hex.fromBytes(a.toBytesBE()),'FFFFFFFFFFFFFFFFFFFFFFFF');
        
        a.set(b);
        assert.equal(hex.fromBytes(a.toBytesBE()),expect);
    })
}

tsts(`set-oversized-throws`,()=>{
    const a=UIntMut.fromUint32Set(0xffffffff);
    assert.throws(()=>a.set(UInt.fromUint32Set(1,2)));
})

tsts(`clone`,()=>{
	//Really just for coverage (since you cannot mutate, what's the value of a clone?)
	const a=UIntMut.fromBytesLE(Uint8Array.of(1,0,0,0),4);
    const b=a.clone();
...
tsts(`zero`,()=>{
    const aHex='00000001000000020000000300000004';
    const aBytes=hex.toBytes(aHex);
    const a=UIntMut.fromBytesBE(aBytes,aBytes.length>>2);
    assert.equal(hex.fromBytes(a.toBytesBE()),aHex);
    a.zero();
    assert.equal(hex.fromBytes(a.toBytesBE()),'00000000000000000000000000000000');
})
