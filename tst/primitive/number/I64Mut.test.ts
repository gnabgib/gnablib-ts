...
for (const [i52, expect] of fromInt_tests) {
	tsts(`fromInt(${i52})`, () => {
		const a = I64Mut.fromInt(i52);
		assert.is(hex.fromBytes(a.toBytesBE()), expect);
	});
	tsts(`fromBytesBE(${expect})`, () => {
		const a = I64Mut.fromBytesBE(hex.toBytes(expect));
		assert.is(hex.fromBytes(a.toBytesBE()), expect);
	});
}
...
for (const bad of bad_fromInt_tests) {
	tsts(`fromInt(${bad}) throws`, () => {
		assert.throws(() => I64Mut.fromInt(bad));
	});
}
...
for (const bad of bad_fromI32s_tests) {
	tsts(`fromI32s(${bad}) throws`, () => {
		assert.throws(() => I64Mut.fromI32s(...bad));
	});
}
...
for (const bad of bad_fromBytesBE_tests) {
	tsts(`fromBytesBE(${bad}) throws`, () => {
		assert.throws(() => I64Mut.fromBytesBE(hex.toBytes(bad)));
	});
}
...
for (const [arr, pos] of bad_mount_tests) {
	tsts(`mount([${arr.length}],${pos}) throws`, () => {
		assert.throws(() => I64Mut.mount(arr, pos));
	});
}
//#endregion
...
for (const [start, by, expect] of lShift_tests) {
	tsts(`${start} << ${by}`, () => {
		...
		const a = I64Mut.fromBytesBE(aBytes);
...
for (const [start, expect] of not_tests) {
	tsts(`~${start}`, () => {
		const a = I64Mut.fromBytesBE(hex.toBytes(start));
		const res = a.notEq();
		assert.is(hex.fromBytes(res.toBytesBE()), expect);
		assert.is(a, res);
	});
...
for (const [aNum, bNum] of abs_tests) {
	...
	const bHex = hex.fromBytes(b.toBytesBE());
	tsts(`abs(${aNum})`, () => {
		const res = a.absEq();
		assert.equal(hex.fromBytes(res.toBytesBE()), bHex);
		assert.is(a, res);
	});
}
//#endregion
...
tsts('set', () => {
	const o = I64Mut.fromInt(1);
	const i1 = I64.fromInt(2);

	assert.is(hex.fromBytes(o.toBytesBE()), '0000000000000001');

	o.addEq(i1);
	assert.is(hex.fromBytes(o.toBytesBE()), '0000000000000003');

	o.set(i1);
	assert.is(hex.fromBytes(o.toBytesBE()), '0000000000000002');
});
...
tsts('zero', () => {
	...
	o.zero();
	assert.is(hex.fromBytes(o.toBytesBE()), '0000000000000000');
});
