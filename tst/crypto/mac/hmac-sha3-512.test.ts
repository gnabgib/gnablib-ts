...
const sha3_512: hashHex[] = [
	...
	},
	{
		...
		data: 'gnabgib',
		expect:
			'D74A71C20A6C941D5FBF165E8DB13496D91F3CD3D2DDD9E860642CCCAACC12E3B7C6FA8BBD8093437691220EDE8BF33A24926336CA6B8204EF2C365A36E7E107',
	},
];
for (const test of sha3_512) {
	...
	tsts('hmac-sha3-512: ' + test.data, () => {
		const bKey =
			test.key instanceof Uint8Array ? test.key : hex.toBytes(test.key);
		const bMsg =
			test.data instanceof Uint8Array ? test.data : utf8.toBytes(test.data);
