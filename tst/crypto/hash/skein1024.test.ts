...
const test_bytes: [string, string][] = [
    ...
    [
        ...
        'F1FBB54F260D0FB9D49A29EEC184B265EDC663668A9720AA61661E43659B3CD6'+
...
for (const [source, size, expect] of test_size) {
	tsts(`Skein-1024-${size}Ƃ(x${source})`, () => {
...
tsts(`New384`,()=>{
    const hash=Skein1024.New384();
    hash.write(zeroBytes);
    assert.equal(
        hex.fromBytes(hash.sum()),
        '1FDB081963B960E89EAA11B87DDA55E8A55A3E1066B30E38D8AE2A45242F7DAD'+
        'FAF06D80CA8A73CD8242CE5EAB84C164'
    );
})

tsts(`New512`,()=>{
    const hash=Skein1024.New512();
    hash.write(zeroBytes);
    assert.equal(
        hex.fromBytes(hash.sum()),
