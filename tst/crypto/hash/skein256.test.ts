...
const test_bytes: [string, string][] = [
	...
	['FF', '0B98DCD198EA0E50A7A244C444E25C23DA30C10FC9A1F270A6637F1F34E67ED2'],
	[
...
tsts(`reset`,()=>{
    ...
    assert.equal(hex.fromBytes(hash.sum()),'C8877087DA56E072870DAA843F176E9453115929094C3A40C463A196C29BF7BA');
    hash.write(hex.toBytes(''));
    assert.equal(hex.fromBytes(hash.sum()),'C8877087DA56E072870DAA843F176E9453115929094C3A40C463A196C29BF7BA');
    hash.write(hex.toBytes('FF'));
    assert.equal(hex.fromBytes(hash.sum()),'0B98DCD198EA0E50A7A244C444E25C23DA30C10FC9A1F270A6637F1F34E67ED2','first sum');
    hash.reset();
...
tsts(`newEmpty`,()=>{
    const hash=new Skein256();
    hash.write(hex.toBytes('FF'));
    assert.equal(hex.fromBytes(hash.sum()),'0B98DCD198EA0E50A7A244C444E25C23DA30C10FC9A1F270A6637F1F34E67ED2');
    const hash2=hash.newEmpty();
    hash2.write(hex.toBytes('FF'));
    assert.equal(hex.fromBytes(hash2.sum()),'0B98DCD198EA0E50A7A244C444E25C23DA30C10FC9A1F270A6637F1F34E67ED2');
});
